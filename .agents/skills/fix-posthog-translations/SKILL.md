...
description: Bulk-fix wrong or missing Armenian vocabulary translations reported via the PostHog Open feedback survey. Use when the user asks to process translation-error feedback from PostHog, fix absent/wrong EN/RU glosses, or patch static/vocabulary.json from survey responses.
---
...
`static/vocabulary.json` is the **curated** dictionary. Patch it in place — do **not** introduce an overrides sidecar for one-off fixes, and do **not** re-run `vocabulary-build` just to change a gloss.

1. Find the lemma (`am`) in `static/vocabulary.json` (any CEFR level array).
2. Set `en` and/or `ru` arrays. **Maximum 5 meanings per language.** Prefer the most common learner-facing senses; drop slang, grammar-form glosses, and rare senses when over the cap.
3. Put the primary learner-facing sense first. Keep real secondary senses (homographs/polysemy) when they fit under the cap.
4. Rebuild search keys:

```bash
bun run search-index-build
```

5. Run `bun run lint` and `bun run build` before declaring done.

Example shape inside a level array:

```json
{
  "am": "տալ",
  "en": ["to give", "to hand over", "to allot", "to contribute", "sister-in-law"],
  "ru": ["давать, дать", "отдавать, отдать", "передавать, передать", "выдавать, выдать", "золовка"]
}
```

If `bun run vocabulary-build` is ever needed for other reasons, the builder **preserves existing en/ru** for lemmas already in `static/vocabulary.json` unless `--replace-all-translations` is passed. See AGENTS.md “Vocabulary curation”.
...
4. Patch all affected entries in `static/vocabulary.json` in one edit.
5. Run `bun run search-index-build` once for the batch.
...
1. **Local repo files (always first)**
   - Current curated data: `static/vocabulary.json`
   - Russian StarDict extract: `scripts/tmp/armenian_russian.csv` (source; often incomplete/wrong)
   - English kaikki cache: `scripts/tmp/kaikki_entries.json` → `english` map; raw senses in `vocabulary_sources/kaikki.org-dictionary-Armenian-words.jsonl` (source; may overwrite senses across POS)
...
4. **https://www.museum.am/glossary/display-armrus.php?action=search&word=<lemma>&type=full&method=1**
   ...
   - Often **very verbose**. Use to confirm meaning; extract at most 5 learner glosses.
...
5. **https://www.armdict.com/dictionary/armenian-russian/<lemma>** (and `/dictionary/armenian-english/<lemma>` for EN)
   - Supporting evidence; coverage uneven.
...
| https://www.polytranslator.com/dictionary/armenian-to-english/ | Static common-word table only; per-lemma paths **404**. |

### Conflict / sense rules

- Prefer **multi-sense** dictionary sources over MT one-liners.
- If sources disagree on the primary sense, check brrn + Glosbe; put the learner-primary sense first (**≤ 5** per language).
...
- Re-running full vocabulary regeneration solely to fix a gloss.
